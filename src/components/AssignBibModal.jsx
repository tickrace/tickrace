...
export default function AssignBibModal({ open, onClose, rows = [], selectedIds = [], onDone }) {
  ...
  // Reset propre à chaque fermeture
  React.useEffect(() => {
    if (!open) {
      setStart(100);
      setEnd(500);
  ...
  // Si sélection vide, on rebascule la portée sur "all"
  React.useEffect(() => {
    if (selectedIds.length === 0 && scope === "selected") setScope("all");
  }, [selectedIds.length, scope]);
  ...
  const capacity = validRange ? (+end - +start + 1) : 0;

  const simulate = () => setAssignCount(validRange ? Math.min(capacity, targetRows.length) : 0);

  const run = async () => {
    if (!validRange) return alert("Plage invalide.");
    if (capacity <= 0) return alert("Plage vide.");
    if (targetRows.length === 0) return alert("Aucun coureur à numéroter.");
    ...
    try {
      ...
      for (let n = +start; n <= +end; n++) {
        if (overwrite || !taken.has(n)) seq.push(n);
      }
      if (seq.length === 0) { setSaving(false); return alert("Aucun numéro disponible dans la plage."); }

      const toAssign = targetRows.slice(0, seq.length);
      const updates = toAssign.map((r, i) =>
        supabase.from("inscriptions").update({ dossard: seq[i] }).eq("id", r.id)
      );
      const results = await Promise.allSettled(updates);
      const ok = results.filter(x => x.status === "fulfilled").length;
      const ko = results.length - ok;
      alert(`Dossards attribués : ${ok}${ko ? ` • Échecs : ${ko}` : ""}`);
      onDone?.();
    } catch (e) {
  ...
  // Toujours monté ; on masque si open=false
  return (
    <div className={cls("fixed inset-0 z-40 p-4", open ? "flex items-end sm:items-center justify-center bg-black/30" : "hidden")}>
