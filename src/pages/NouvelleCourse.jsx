...
export default function NouvelleCourse() {
  ...
  const geocodeLieu = async (lieu, code_postal) => {
    const query = encodeURIComponent(`${lieu} ${code_postal} France`);
    const url = `https://nominatim.openstreetmap.org/search?format=json&q=${query}`;
    try {
      const response = await fetch(url, { headers: { "User-Agent": "Tickrace" } });
  ...
  const handleSubmit = async (e) => {
    ...
    // --- Géocodage ---
    const { lat, lng } = await geocodeLieu(course.lieu, course.code_postal);
