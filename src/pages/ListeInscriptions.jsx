...
");
  ...
  // Formats disponibles pour l'export (liste affichée / filtre actif)
  const exportableFormats = useMemo(() => {
    if (formatId) {
      const f = formatsById[formatId] || formatsList.find((x) => x.id === formatId);
      return f ? [f] : [];
    }
    // sinon, tous les formats connus (de la course)
    return formatsList;
  ...
  return (
    <div className="mx-auto max-w-7xl p-4">
      ...
      <div className="sticky top-0 z-10 mb-4 rounded-2xl border border-neutral-200 bg-white/95 p-3 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <Input
              placeholder="Recherche : nom, prénom, email, équipe"
              value={search}
            ...
            <Select value={formatId} onChange={(e) => setFormatId(e.target.value)} style={{ minWidth: 220 }}>
              ...
              {formatsList.map((f) => (
                <option key={f.id} value={f.id}>
                  {f.nom}{f.distance_km ? ` • ${f.distance_km}km` : ''}{f.denivele_dplus ? ` • +${f.denivele_dplus}m` : ''}
                </option>
              ))}
            </Select>
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" className="rounded" checked={onlyCompleteTeams} onChange={(e) => setOnlyCompleteTeams(e.target.checked)} />
              Uniquement équipes complètes
            </label>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Select
              value={`${orderBy.column}:${orderBy.ascending ? "asc" : "desc"}`}
              onChange={(e) => {
                const [col, dir] = e.target.value.split(":");
                setOrderBy({ column: col, ascending: dir === "asc" });
              }}
            >
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && filteredRows.length === 0 && (
              <tr>
                <td colSpan={10} className="p-8 text-center text-neutral-400">Aucune inscription.</td>
              </tr>
            )}
              ...
              <tr key={r.id} className="border-t">
                ...
                <td className="p-3 align-top">
                  <div className="flex flex-col gap-1">
                    ...
                    {r.is_waitlist ? <Badge tone="blue">liste d'attente</Badge> : null}
                  </div>
                </td>
      ...
      {/* Erreur */}
      {error && (
        <div className="mt-3 rounded-xl border border-rose-300 bg-rose-50 p-3 text-rose-700">{error}</div>
      )}
      ...
      <Modal open={emailOpen} onClose={() => setEmailOpen(false)} title={`Envoyer un email (${audienceEmails.length} destinataire${audienceEmails.length > 1 ? "s" : ""})`}>
        <div className="space-y-3">
          ...
          <div className="text-xs text-neutral-500">
            Les emails seront envoyés à : {audienceEmails.slice(0, 5).join(", ")}{audienceEmails.length > 5 ? `, +${audienceEmails.length - 5}…` : ""}
          </div>
        </div>
        <div className="mt-4 flex items-center justify-end gap-2">
          <Button onClick={() => setEmailOpen(false)} className="border-neutral-300">Annuler</Button>
          <Button onClick={sendEmail} disabled={emailSending || audienceEmails.length === 0} className="bg-orange-500 text-white hover:bg-orange-600">
      ...
      <Modal open={exportOpen} onClose={() => setExportOpen(false)} title="Exporter en CSV" widthClass="max-w-xl">
        ...
        <div className="mt-3 grid gap-2">
          {exportableFormats.length === 0 && (
            <div className="rounded-xl border border-neutral-200 p-3 text-sm text-neutral-500">Aucun format disponible.</div>
          )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <div>{statutBadge(teamPanelGroup.statut)}</div>
              </div>
              <div>
                <div className="text-neutral-500">Catégorie</div>
                <div>{teamPanelGroup.team_category || "—"}</div>
              </div>
              <div>
                <div className="text-neutral-500">Taille</div>
                <div>{teamPanelGroup.members_count || 0}/{teamPanelGroup.team_size || 0}</div>
              </div>
              <div>
                <div className="text-neutral-500">Format</div>
                <div>{formatLabel(teamPanelGroup.format_id)}</div>
              </div>
            </div>
              ...
              <div className="overflow-hidden rounded-xl border">
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    ...
                    {teamPanelMembers.length === 0 && (
                      <tr><td colSpan={6} className="p-3 text-center text-neutral-500">Aucun membre.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      <div className="mt-8 text-xs text-neutral-400">
        <p>
          NB : le sélecteur de formats se base sur <code>courseId</code>. Si tu veux lister tous les formats de l'organisateur (multi-courses),
          remplace la requête par un <code>select</code> filtré via un lien <code>courses.organisateur_id = auth.uid()</code>.
        </p>
      </div>
    </div>
  );
