...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    // ✅ IMPORTANT :
    // si la confirmation email est activée, l’utilisateur ne sera pas “connecté” immédiatement.
    // on le redirige vers Login en conservant next, et après connexion -> retour au formulaire.
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: {
        // si tu as AuthCallback, tu peux aussi le brancher ici plus tard :
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-50 px-4">
      <div className="max-w-md w-full text-center">
        <h1 className="text-3xl font-extrabold text-neutral-900 mb-2">Créer un compte</h1>
        <p className="text-sm text-neutral-600 mb-6">
          Un compte est nécessaire pour accéder à ton espace (planning, missions, chat, suivi).
        </p>
        ...
        <div className="mt-6 text-center">
          <Link to={`/login?next=${encodeURIComponent(nextPath)}`} className="text-neutral-700 hover:underline text-sm">
