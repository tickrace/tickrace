...
function Pill({ children, color = "neutral" }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${map[color]}`}>
      {children}
    </span>
  );
}
...
export default function MonInscription() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      // 3a. paiements liés directement
      const { data: paysDirect } = await supabase.from("paiements").select("*").eq("inscription_id", id).order("created_at", { ascending: false });
      // 3b. paiements groupés où inscription_ids contient id — selon ton schéma c'est un ARRAY typé
      const { data: paysGroup } = await supabase
        .from("paiements")
        .select("*")
        .contains("inscription_ids", [id]) // fonctionne si ARRAY uuid/text
  ...
  const onAnnuler = async () => {
    ...
    try {
      ...
      for (const args of tryArgs) {
        const { data, error } = await supabase.rpc("calculer_credit_annulation", args);
        rpcRes = data;
        if (!error) {
          rpcErr = null;
          break;
  ...
  const OptionsBloc = () => {
    ...
    if (!hasA && !hasB) return <div className="text-sm text-neutral-500">Aucune option.</div>;

    return (
      <div className="grid gap-3">
        {hasA && (
          <div>
            <div className="text-sm font-semibold text-neutral-700 mb-2">Options (catalogue “format_options”)</div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {insc._optionsA.map((o) => {
                    ...
                    const unit = o?.unit_price_cents != null ? o.unit_price_cents / 100 : (o?.format_option?.prix_cents ?? 0) / 100;
                    const total = o?.total_cents != null ? o.total_cents / 100 : unit * (o?.quantity ?? 1);
                    return (
                      <tr key={`A-${o.id}`} className="border-t border-neutral-200">
        ...
        {hasB && (
          <div>
            <div className="text-sm font-semibold text-neutral-700 mb-2">Options (catalogue “options_catalogue”)</div>
            <div className="rounded-xl ring-1 ring-neutral-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-neutral-50">
                  <tr className="text-left">
                ...
                <tbody>
                  {insc._optionsB.map((o) => {
                    ...
                    const unit = (o?.prix_unitaire_cents ?? o?.option?.price_cents ?? 0) / 100;
                    const q = o?.quantity ?? 1;
                    const total = unit * q;
                    return (
                      <tr key={`B-${o.id}`} className="border-t border-neutral-200">
                        <td className="px-3 py-2">{label}</td>
                        <td className="px-3 py-2">{q}</td>
                        <td className="px-3 py-2">{euros(unit)}</td>
                        <td className="px-3 py-2">{euros(total)}</td>
                        <td className="px-3 py-2">
                          <Pill color={o?.status === "confirmed" ? "green" : o?.status === "canceled" ? "red" : "orange"}>
                            {o?.status || "pending"}
                          </Pill>
                        </td>
                      </tr>
  ...
  if (loading) return <div className="min-h-screen bg-neutral-50 p-8">Chargement…</div>;
  if (error) return (
  ...
  if (!insc) return <div className="min-h-screen bg-neutral-50 p-8">Inscription introuvable.</div>;

  const course = insc.courses;
  const format = insc.formats;
  const tz = format?.fuseau_horaire || "Europe/Paris";
  const isCanceled = !!insc.cancelled_at || (insc.statut || "").toLowerCase().includes("annul");
  ...
  const totalTheo = totalCoureur + totalRepas;
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <section className="bg-white border-b border-neutral-200">
        <div className="mx-auto max-w-7xl px-4 py-10 flex flex-col items-center text-center gap-3">
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight">
            Mon inscription <span className="font-black"><span className="text-orange-600">Tick</span>Race</span>
          </h1>
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 grid gap-8">
        ...
        <Card
          ...
          subtitle={course ? `${course.lieu || "—"} • ${course.departement || ""}` : "—"}
          right={
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-xl ring-1 ring-neutral-200 p-4 bg-neutral-50">
              <div className="text-sm font-semibold text-neutral-700 mb-2">Détails du format</div>
              <Row label="Format">{format?.nom || "—"}</Row>
              <Row label="Type">{format?.type_epreuve || "—"} {format?.type_format ? `• ${format.type_format}` : ""}</Row>
              <Row label="Date / Heure">
                {format?.date ? `${formatDate(format.date)} • ${format?.heure_depart ? format?.heure_depart.slice(0,5) : "—"}` : "—"}
                {format?.fuseau_horaire ? ` (${format.fuseau_horaire})` : ""}
              </Row>
              <Row label="Distance">{km(format?.distance_km)}</Row>
              <Row label="D+ / D-">
                {meters(format?.denivele_dplus)} / {meters(format?.denivele_dmoins)}
              </Row>
              {insc.dossard != null && <Row label="Dossard">#{insc.dossard}</Row>}
              {insc.team_name && <Row label="Équipe">{insc.team_name}</Row>}
            </div>

            <div className="rounded-xl ring-1 ring-neutral-200 p-4 bg-neutral-50">
              <div className="text-sm font-semibold text-neutral-700 mb-2">Mon profil</div>
              ...
              {insc.pps_identifier && (
                <Row label="PPS">{insc.pps_identifier} {insc.pps_expiry_date ? `(valide jusqu’au ${formatDate(insc.pps_expiry_date)})` : ""}</Row>
              )}
              {insc.contact_urgence_nom && (
                <Row label="Contact urgence">
                  {insc.contact_urgence_nom} • {insc.contact_urgence_telephone || "—"}
                </Row>
              )}
              {insc.cancelled_at && (
                <Row label="Annulée le">{formatDateTime(insc.cancelled_at, tz)}</Row>
              )}
        ...
        <Card title="Options & repas">
          <div className="grid gap-6">
            <div className="rounded-xl ring-1 ring-neutral-200 p-4 bg-neutral-50">
              <div className="text-sm font-semibold text-neutral-700 mb-2">Restauration</div>
              <Row label="Repas réservés">{insc.nombre_repas ?? 0}</Row>
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-xl ring-1 ring-neutral-200 p-4 bg-neutral-50">
              <div className="text-sm font-semibold text-neutral-700 mb-2">Récapitulatif</div>
              <Row label="Sous-total coureur">{euros(totalCoureur)}</Row>
              <Row label="Restauration">{euros(totalRepas)}</Row>
              <Row label="Total">{euros(totalTheo)}</Row>
              <Row label="Statut">{insc.statut || "—"}</Row>
            </div>
            <div className="rounded-xl ring-1 ring-neutral-200 p-4 bg-neutral-50">
              <div className="text-sm font-semibold text-neutral-700 mb-2">Transactions</div>
              {payInfos.paiements?.length ? (
                    ...
                    <tbody>
                      {payInfos.paiements.map((p) => (
                        <tr key={p.id} className="border-t border-neutral-200">
                          ...
                          <td className="px-3 py-2">
                            <Pill color={p.status === "succeeded" || p.status === "paid" ? "green" : p.status === "pending" ? "orange" : "red"}>
                              {p.status || "—"}
                            </Pill>
                          </td>
                        </tr>
                ...
                <div className="text-sm text-neutral-500">Aucune transaction affichée.</div>
              )}
            </div>
          </div>
        </Card>
        ...
        <Card title="Crédit d’annulation">
          {credit ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="rounded-xl ring-1 ring-neutral-200 p-4 bg-neutral-50">
                <div className="text-sm font-semibold text-neutral-700 mb-2">Résumé</div>
                <Row label="Date d’annulation">{formatDateTime(credit.created_at)}</Row>
                <Row label="Jours avant course">{credit.jours_avant_course ?? "—"}</Row>
                <Row label="% conservé par l’orga">{credit.pourcentage_conserve != null ? `${Number(credit.pourcentage_conserve)} %` : "—"}</Row>
                {credit.pourcentage != null && <Row label="% remboursé">{`${Number(credit.pourcentage)} %`}</Row>}
              </div>
              <div className="rounded-xl ring-1 ring-neutral-200 p-4 bg-neutral-50">
                <div className="text-sm font-semibold text-neutral-700 mb-2">Montants</div>
                <Row label="Remboursement inscription">{euros(credit.remboursement_inscription)}</Row>
                <Row label="Remboursement repas">{euros(credit.remboursement_repas)}</Row>
                <Row label="Frais d’annulation">{euros(credit.frais_annulation)}</Row>
                <Row label="Total remboursé">{euros(credit.montant_rembourse ?? credit.montant_total)}</Row>
              </div>
            </div>
          ) : (
            <div className="text-sm text-neutral-500">
              ...
              {!isCanceled && (
                <> Vous pouvez utiliser le bouton <strong>“Annuler mon inscription”</strong> ci-dessus.</>
              )}
            </div>
          )}
        </Card>
