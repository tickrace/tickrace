...
export default async function handler(req: Request) {
  ...
  if (!id) {
    return new ImageResponse(
      <div style={{ width: 1200, height: 630, display: "flex", alignItems: "center", justifyContent: "center", background: DARK, color: "white", fontSize: 56, fontWeight: 900 }}>
        Tickrace
      </div>,
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ width: 10, height: 10, borderRadius: 999, background: ORANGE, boxShadow: "0 0 0 6px rgba(249,115,22,.15)" }} />
            <div style={{ fontSize: 18, opacity: 0.88 }}>Inscriptions & résultats</div>
          </div>

          <div style={{ padding: "12px 16px", borderRadius: 999, background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.14)", fontSize: 18 }}>
            <span style={{ color: ORANGE, fontWeight: 900 }}>●</span> tickrace.com
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
          {topChips.length ? (
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
              {topChips.map((c) => (
                <div key={c} style={{ padding: "10px 14px", borderRadius: 999, background: "rgba(249,115,22,.14)", border: "1px solid rgba(249,115,22,.28)", fontSize: 20 }}>
                  {c}
                </div>
              ))}
            </div>
          ...
          <div style={{ fontSize: 72, fontWeight: 900, lineHeight: 1.05, letterSpacing: -1, textShadow: "0 10px 30px rgba(0,0,0,.40)", marginBottom: 18, maxWidth: 1040 }}>
            {title}
          </div>

          {bottomLine ? <div style={{ fontSize: 24, opacity: 0.92, textShadow: "0 10px 30px rgba(0,0,0,.40)" }}>{bottomLine}</div> : null}
        </div>
      </div>
    ),
    { width: 1200, height: 630 }
