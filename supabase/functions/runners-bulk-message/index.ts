 ...
 * Optionnel mais recommandé: RESEND_FROM (ex: "Tickrace <contact@tickrace.com>")
 */
...
serve(async (req) => {
  ...
  try {
    // Auth côté code (laisse passer OPTIONS)
    ...
    // 5) Cibles
    type Target = {
      inscription_id: string; email: string; prenom: string; nom: string;
      course: string; format: string; lieu: string; dossard: string;
    };
    ...
    // 7) Envoi Resend (FROM sûr + logs d'erreur)
      ...
      const results = await Promise.allSettled(part.map(async (t) => {
        ...
        try {
          await resend.emails.send({
            from: RESEND_FROM,           // <= sécurisé via secret (fallback onboarding@resend.dev)
            to: t.email,
            subject: subj,
            html,
            text,
    ...
    return json({
      ...
      sent, failed,
      errors, // <= tu verras la cause exacte s'il y en a
    }, 200, origin);
