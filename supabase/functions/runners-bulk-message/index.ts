...
serve(async (req) => {
  ...
  try {
    ...
    // 1) Récup inscriptions (schéma: nom, prenom, email directement dans la table)
    ...
    const permittedBase = Array.isArray(include_statut) && include_statut.length
      ? (rows || []).filter(r => include_statut.includes(r.statut))
      : (rows || []);

    // 2) Récup formats -> course_id, nom format
    const formatIds = Array.from(new Set(permittedBase.map(r => r.format_id).filter(Boolean)));
    ...
    // 3) Récup courses -> autorisation + nom/lieu
    ...
    const permitted = permittedBase.filter((r) => {
    ...
    if (dry_run) {
      return json({
        ...
        permitted: permitted.length,                  // après contrôle d'autorisation
        permitted_after_filters: permitted.length,    // (filtre statut déjà appliqué plus haut)
