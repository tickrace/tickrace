...
serve(async (req) => {
  ...
  try {
    ...
    // 1) Récupérer la charge Stripe liée à l’inscription
    ...
    // 2) Calcul du montant à rembourser (95 % sur la part orga)
    //    -> si tu as déjà ta fonction SQL, on l’appelle, sinon on reconstruit ici.
    ...
    // Attendu: creditRow.montant_rembourse déjà en € selon ta règle (95% orga)
    ...
    const refund = await stripe.refunds.create({
      ...
      amount: amount_cents,
      reverse_transfer: true,        // récupère sur le compte connecté
      refund_application_fee: false, // ne rembourse PAS la commission Tickrace (5%)
    });

    // 4) Mettre l’inscription à "remboursé" + lien vers paiement/credit si besoin
    await supabase.from("inscriptions").update({ statut: "remboursé" }).eq("id", inscription_id);
