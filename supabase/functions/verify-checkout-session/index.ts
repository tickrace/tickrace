...
/* ------------------------------ Core updater ------------------------------ */
async function finalizePaymentFromSession(session: Stripe.Checkout.Session, sendEmail: boolean) {
  const sessionId = session.id;

  // 1) Retrouver/constituer la liste des inscriptions liées à la session
  let pay = await supabase
    .from("paiements")
    .select("id, inscription_ids, status, total_amount_cents")
  ...
  // 2) Marquer le paiement comme "paye" (upsert/update)
  if (pay.data) {
    ...
    // Rare: si la ligne paiement n'existe pas, on l'insère (sans total exact)
  ...
  // 3) Mettre à jour inscriptions / groupes / options
  if (inscriptionIds?.length) {
  ...
  if (sendEmail) {
    ...
    const html = `
      <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif">
        ...
        <p style="color:#667085;font-size:12px">Session Stripe : ${session.id}</p>
      </div>
    `;
    ...
    if (!r.ok && !payerEmail && inscriptionIds?.length) {
      // fallback vers le 1er email d'inscription
  ...
  return { inscriptionIds, emailSent };
}
...
serve(async (req) => {
  ...
  try {
    ...
    // 1) Récupérer la session chez Stripe
    ...
    if (!isPaid) {
      return json({ ok: true, paid: false, session_id: sessionId, reason: "not_paid_yet" });
    }

    // 3) Finaliser en base + email
    const result = await finalizePaymentFromSession(session, sendEmail);

    return json({ ok: true, paid: true, session_id: sessionId, ...result });
  } catch (e) {
    console.error("VERIFY_CHECKOUT_FATAL", e);
    return json({ error: "verify_failed", details: String(e?.message ?? e) }, 500);
  }
});
